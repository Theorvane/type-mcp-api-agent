...
class SkillReleaseTests(unittest.TestCase):
    ...
    def test_version_extraction_step_executes_and_writes_outputs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(output_path.read_text(encoding="utf-8"), "skill_version=0.2.5\ntag=v0.2.5\n")
        ...
        for invalid_version in ("0.1.0-01", "0.1.0-alpha.01"):
            with self.subTest(version=invalid_version), tempfile.TemporaryDirectory() as directory:
                ...
                skill_path.write_text(
                    original.replace("version: 0.2.5", f"version: {invalid_version}"),
                    encoding="utf-8",
                )
                previous_cwd = Path.cwd()
                previous_output = os.environ.get("GITHUB_OUTPUT")
    ...
    def test_existing_unpublished_skill_is_recovered_before_version_reconciliation(self) -> None:
        ...
        responses = iter(
            [
                ...
                (200, {"slug": "api-to-typemcp", "status": "PUBLISHED", "latestVersion": "0.2.5"}),
                (200, [{"version": "0.2.5"}]),
                (200, {"slug": "api-to-typemcp", "status": "PUBLISHED", "latestVersion": "0.2.5"}),
            ]
        )
    ...
    def test_clawhub_pending_publication_is_reconciled_without_a_second_publish(self) -> None:
        ...
        payload = waiter.wait_for_publication(
            ...
            attempts=2,
            delay_seconds=0,
            opener=Mock(side_effect=[pending, Response({"version": {"version": "0.2.5"}})]),
            sleep=Mock(),
        )
        self.assertEqual(payload["version"], {"version": "0.2.5"})
